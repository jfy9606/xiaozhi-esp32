#include "location_content.h"

// 位置定位页面HTML内容
static const char location_html[] = R"(
<!DOCTYPE html>
<html>
<head>
  <meta charset='UTF-8'>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function connectWebSocket() {
        ...
        socket.onopen = function() {
          ...
          statusCheckInterval = setInterval(requestLocationUpdate, 1000);
        };
        ...
        socket.onmessage = function(event) {
          try {
            ...
            console.error('解析WebSocket消息出错:', e, event.data);
          }
        };
        ...
        socket.onclose = function() {
          ...
          if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
            ...
            console.log(`${delay}ms后尝试重新连接 (尝试 ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})...`);
      ...
      function handleWebSocketMessage(data) {
        ...
        } else if (data.type === 'map_saved') {
          // 显示地图保存成功的消息
          debugInfo.textContent = `地图保存成功: ${data.path}`;
        } else {
      ...
      function updateLocationInfo(data) {
        ...
        currentPosition = {
          x: data.position.x || 0,
          y: data.position.y || 0,
          orientation: data.position.orientation || 0,
          accuracy: data.position.accuracy || 0
        };
        ...
        // 更新定位模式文本
        modeText.textContent = `当前模式: ${getModeName(data.mode || currentMode)}`;
      }
      ...
      function updateSystemStatus(data) {
        ...
        if (data.sensors && data.sensors.location) {
          // 更新额外的位置传感器信息
          debugInfo.textContent = `更新时间: ${new Date().toLocaleTimeString()}`;
        }
      }
